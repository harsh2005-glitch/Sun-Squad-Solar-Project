...
const getDashboardStats = async (req, res) => {
    try {
        ...
        const pendingUsers = await User.countDocuments({ role: 'associate', status: 'pendingOnboarding' });
        ...
        const recentTransactions = await Transaction.find({})
            .sort({ createdAt: -1 })
            .limit(5)
            .populate('user', 'name associateId');
...
const addDeposit = async (req, res) => {
    ...
    try {
        ...
        // --- FIX: Create Deposit Record as well ---
        await Deposit.create({
            depositor: depositor._id,
            amount: depositAmount,
            approvedBy: adminId,
            isProcessedForCommission: true
        });
...
// @desc    Reset all financial data (Transactions, Deposits, Commissions, User Balances)
// @route   POST /api/admin/reset-financials
// @access  Admin (Super Admin ideally)
const resetSystemFinancials = async (req, res) => {
    try {
        ...
        res.json({ message: 'All financial data has been successfully reset.' });
...
// @desc    Delete a user and all their related financial records
...
const deleteUser = async (req, res) => {
    ...
    try {
        ...
        // 1. Delete Transactions where user is the subject
        await Transaction.deleteMany({ user: userId });

        // 2. Delete Deposits made by the user
        await Deposit.deleteMany({ depositor: userId });

        // 3. Delete Commissions received by the user OR generated by the user
        await Commission.deleteMany({ $or: [{ recipient: userId }, { sourceUser: userId }] });

        // 4. Remove user from their sponsor's directs list
        if (user.sponsor) {
